...
Table 1 describes a number of important initiatives on semantic interoperability. While it is important to foster such developments, there is a need for convergence in order to avoid the following risks:
...
| Large Triple(Quad) Stores | Inferences |
|----|----|
...
Schema.org. Schema.org is a well-known schema catalog to structure data on Web pages to describe the location, person, etc. The IoT Schema.org extension [9] is planned; discussions are ongoing. | 
